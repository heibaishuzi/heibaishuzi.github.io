            ...
            for (var i = 0, len = user_music_list.length; i < len; i++) {
                if (user_music_list[i].split("-").length == 2) {
                    song_data.push({
                        ...
                        songUrl: 'https://cdn.jsdelivr.net/gh/heibaishuzi/music@latest/songs/' + user_music_list[i],
                        imageUrl: 'https://heibaishuzi.github.io/static/music_data/images/' + bgp[Math.floor(Math.random() * bgp.length)]
                    })
                } else {
                    song_data.push({
                        fileName: user_music_list[i],
...
class PlayerCreator {
    ...
    renderSongList() {
        ...
        this.musics.songs.forEach((song, i) => {
            _str += `<li class="music__list__item">${song.title} <a href="https://heibaishuzi.github.io/music/songs/${song.fileName}?raw=true" style="float:right;padding-right:12px">下载</a></li>`
        });
